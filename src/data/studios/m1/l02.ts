import type { StudioDefinition } from '../../../features/studio/types';
import { STUDIO_EXPRESSION_MODES, STUDIO_SUPPORT_PROFILES, sceneBeats } from '../shared';

export const M1_L2_STUDIO: StudioDefinition = {
    id: 'm1-feature-investigation',
    lessonId: 'm1-l2',
    moduleId: 'm1',
    title: '기계와 인공지능은 어떻게 다를까요?',
    subtitle: '기계가 움직이는 방법을 살펴보고, 인공지능이 쓰인 곳을 찾아봐요.',
    format: 'D',
    decisionTitle: '아이미에게 쉽게 물어봐요.',
    suggestedQuestions: [
      '인공지능은 어떻게 대답을 만드는 거야?',
      '인공지능에게 공부에 대해 질문해도 돼?',
      '컴퓨터는 사람처럼 생각할 수 있어?',
    ],
    visualNovel: {
      title: '동아리방의 세 가지 도구',
      objective: '버튼 선풍기·센서 자동문·추천 앱이 받는 정보를 비교해, AI 기능이 쓰인 것을 이유와 함께 골라요.',
      seasonTag: '[아이미가 왔다 · 2화] 선풍기·자동문·음악 앱',
      nextEpisodeHook: '다음 시간 — 아이미가 급식 메뉴를 발표한대요.',
      scenes: [
        {
          id: 'three-tools',
          label: '장면 1 · 세 가지 도구',
          imageSrc: '/lessons/story/m1/m1-l2-scene-01.webp',
          alt: '윤아가 선풍기와 자동문과 음악 추천 앱을 함께 살펴보는 장면',
          knowledgeStep: 0,
          copy: sceneBeats(
            [
              [
                '아이미가 동아리방에 온 다음 날입니다. 아이미는 방 안을 신기한 듯 둘러보다가 문득 궁금한 점이 생겼습니다.',
                '아이미가 동아리방을 찾아온 다음 날입니다. 아이미는 방 안을 신기한 표정으로 찬찬히 둘러보다가, 스스로 움직이는 물건들을 보고 문득 큰 궁금증이 생겼습니다.',
                '아이미가 동아리방을 찾아온 다음 날 방과 후입니다. 아이미는 방 안을 신기한 눈빛으로 찬찬히 둘러보다가, 스스로 알아서 움직이는 여러 기계를 보고 문득 큰 궁금증이 생겼습니다.',
              ],
              [
                '윤아가 책상 위에 세 가지를 나란히 놓았습니다. 버튼을 누르는 선풍기, 센서 자동문, 음악 추천 앱이었습니다.',
                '윤아가 책상 위에 세 가지 도구를 나란히 놓았습니다. 손으로 버튼을 누르는 선풍기, 센서로 사람을 감지하는 자동문, 그리고 음악을 골라 주는 추천 앱이었습니다.',
                '윤아가 교실 책상 위에 세 가지 도구를 나란히 올려놓았습니다. 손으로 버튼을 누르는 선풍기, 센서로 사람을 감지해 열리는 자동문, 그리고 취향에 맞는 음악을 골라 주는 추천 앱이었습니다.',
              ],
              [
                '아이미: "셋 다 혼자서 움직이는데… 저랑 도대체 어떤 점이 다른가요?" 진우와 윤아는 선뜻 대답하지 못했습니다.',
                '아이미가 푸른 두 눈을 깜빡이며 물었습니다. 아이미: "셋 다 혼자서 척척 움직이는데… 저랑 도대체 어떤 점이 다른가요?" 진우와 윤아는 선뜻 대답하지 못했습니다.',
                '아이미가 푸른 두 눈을 깜빡거리며 고개를 갸웃했습니다. 아이미: "셋 다 혼자서 척척 움직이는데… 저랑 도대체 어떤 점이 다른가요?" 진우와 윤아는 알쏭달쏭해서 선뜻 대답하지 못했습니다.',
              ],
            ],
            '움직이는 모습만 보면 모두 똑똑한 기계처럼 느껴질 수 있어요.',
          ),
        },
        {
          id: 'first-sort',
          label: '장면 2 · 첫 번째 분류',
          imageSrc: '/lessons/story/m1/m1-l2-scene-02.webp',
          alt: '진우가 스스로 움직이면 전부 AI라고 성급하게 말하고 윤아가 갸웃하는 장면',
          knowledgeStep: 0,
          copy: sceneBeats(
            [
              [
                '진우가 자신만만하게 손을 들었습니다. 진우: "그거야 간단하지! 전기가 통하고 저절로 움직이면 전부 다 인공지능이야!"',
                '진우가 자신만만한 표정으로 손을 번쩍 들었습니다. 진우: "그거야 아주 간단하지! 전기가 통하고 저절로 움직이면 선풍기든 자동문이든 전부 다 인공지능이야!"',
                '진우가 자신만만한 표정으로 손을 번쩍 치켜들었습니다. 진우: "그거야 아주 간단하지! 전기가 통하고 사람 손 없이 알아서 척척 움직이면 선풍기든 자동문이든 전부 다 똑같은 최신 인공지능이지!"',
              ],
              [
                '윤아는 고개를 갸우뚱했습니다. 윤아: "정말 그럴까? 선풍기는 사람이 손으로 버튼을 꼭 눌러야만 돌아가잖아."',
                '윤아는 고개를 가만히 갸우뚱했습니다. 윤아: "정말 그럴까? 선풍기는 사람이 손으로 버튼을 꼭 눌러야만 돌아가잖아. 그걸 스스로 생각해서 움직인다고 할 수 있을까?"',
                '윤아는 턱을 괴고 고개를 가만히 갸우뚱했습니다. 윤아: "정말 그럴까? 선풍기는 사람이 손으로 강풍 버튼을 꼭 눌러야만 돌아가잖아. 그걸 기계가 스스로 생각해서 움직인다고 할 수 있을까?"',
              ],
              [
                '진우: "어… 듣고 보니 그렇네." 움직인다는 겉모습만 보고는 셋을 어떤 기준으로 나누어야 할지 알 수 없었습니다.',
                '진우: "어… 듣고 보니 그렇네." 진우는 말문이 턱 막혔습니다. 움직인다는 겉모습만 보고는 세 가지 도구를 어떤 기준으로 나누어야 할지 도무지 알 수 없었습니다.',
                '진우: "어… 듣고 보니 그렇네." 진우는 머리를 긁적이며 말문이 턱 막혔습니다. 움직인다는 겉모습만 보고는 세 가지 도구를 대체 어떤 기준으로 똑똑하게 나누어야 할지 도무지 알 수 없었습니다.',
              ],
            ],
            '처음 생각은 새 정보를 보기 전의 소중한 출발점이에요.',
          ),
        },
        {
          id: 'signal-board',
          label: '장면 3 · 입력 신호 조사',
          imageSrc: '/lessons/story/m1/m1-l2-scene-03.webp',
          alt: '윤아의 확인 질문으로 버튼 센서 사용기록의 차이를 하나씩 찾아보는 장면',
          knowledgeStep: 1,
          copy: sceneBeats(
            [
              [
                '윤아가 공책에 표를 그렸습니다. 윤아: "기계가 어떤 정보를 받아서 움직이는지, 들어오는 신호부터 하나씩 살펴보자."',
                '윤아가 공책에 반듯하게 표를 그렸습니다. 윤아: "기계가 무엇을 받아서 작동하는지 살펴보자. 선풍기는 전깃줄 말고 어떤 신호를 받지?" 진우: "그건 사람이 누르는 버튼이지!"',
                '윤아가 공책에 반듯하게 세 칸짜리 표를 그렸습니다. 윤아: "도구가 무엇을 받아서 작동하는지 살펴보자. 선풍기는 어떤 신호를 받지?" 진우: "그거야 사람이 손가락으로 누르는 버튼 신호지!"',
              ],
              [
                '윤아: "그럼 현관 자동문은?" 자동문에는 버튼이 없습니다. 문 앞 센서가 사람의 움직임을 감지해서 문을 엽니다.',
                '윤아: "그럼 현관 자동문은 뭘 받을까?" 자동문에는 누르는 버튼이 없습니다. 대신 문 위에 달린 센서가 사람이 다가오는 그림자나 움직임을 알아채고 문을 열어 줍니다.',
                '윤아: "그럼 현관 자동문은 어떤 신호를 받을까?" 자동문에는 손으로 누르는 버튼이 없습니다. 대신 문 위에 달린 센서가 사람이 다가오는 움직임을 저절로 알아채고 스르륵 문을 열어 줍니다.',
              ],
              [
                '음악 추천 앱은 또 달랐습니다. 내가 지난번에 들었던 노래 기록을 모아서 좋아할 만한 다음 노래를 스스로 골라 줍니다.',
                '스마트폰 속 음악 추천 앱은 또 완전히 달랐습니다. 내가 지난번에 자주 들었던 노래 기록을 꼼꼼히 모아서, 내가 다음에 좋아할 만한 새로운 노래를 스스로 골라 추천해 줍니다.',
                '스마트폰 속 음악 추천 앱은 작동 방식이 또 완전히 달랐습니다. 내가 지난번에 자주 들었던 노래 기록을 꼼꼼히 모아서 분석한 뒤, 내가 다음에 좋아할 만한 새로운 노래를 스스로 골라 추천해 줍니다.',
              ],
            ],
            '기기 이름보다 그 기능이 무엇을 입력받는지 살펴봐요.',
          ),
        },
        {
          id: 'feature-map',
          label: '장면 4 · 저는 어느 쪽인가요?',
          imageSrc: '/lessons/story/m1/m1-l2-scene-04.webp',
          alt: '아이미가 빈 지도를 함께 채우자며 학생에게 묻는 장면',
          knowledgeStep: 2,
          copy: sceneBeats(
            [
              [
                '표가 완성되었습니다. 선풍기는 버튼, 자동문은 센서, 음악 앱은 사용 기록을 받았습니다. 셋이 받는 정보가 달랐습니다.',
                '공책 속 표가 깔끔하게 완성되었습니다. 선풍기는 손가락 버튼, 자동문은 움직임 센서, 음악 앱은 사용 기록을 받았습니다. 셋이 받는 정보와 일하는 방식이 완전히 달랐습니다.',
                '공책 속 표가 마침내 깔끔하게 완성되었습니다. 선풍기는 손가락 버튼, 자동문은 움직임 센서, 음악 추천 앱은 차곡차곡 쌓인 사용 기록을 받았습니다. 셋이 받는 정보와 일하는 방식이 완전히 달랐습니다.',
              ],
              [
                '진우: "받는 정보가 다르면 일하는 방식도 다르구나!" 하지만 셋 중에 진짜 인공지능이 무엇인지는 아직 몰랐습니다.',
                '진우: "받는 정보가 다르면 일하는 방식도 이렇게 크게 달라지는구나!" 진우는 감탄했지만, 세 가지 도구 중에 진짜 인공지능 기술이 쓰인 것이 무엇인지는 여전히 헷갈렸습니다.',
                '진우: "받는 신호와 정보가 다르면 일하는 방식도 이렇게 크게 달라지는구나!" 진우는 고개를 끄덕이며 감탄했지만, 세 가지 도구 중에 진짜 인공지능 기술이 쓰인 것이 무엇인지는 여전히 헷갈렸습니다.',
              ],
              [
                '아이미: "세 가지 도구 중 인공지능 기능이 들어간 것은 무엇일까요? 여러분은 어떤 이유로 그렇게 생각하시나요?"',
                '아이미가 표 앞으로 다가와 여러분을 바라보았습니다. 아이미: "세 가지 도구 중 진짜 인공지능 기능이 들어간 것은 무엇일까요? 여러분은 어떤 까닭으로 그렇게 생각하시나요?"',
                '아이미가 표 앞으로 조용히 다가와 여러분을 초롱초롱한 눈으로 바라보았습니다. 아이미: "세 가지 도구 중 진짜 인공지능 기능이 들어간 것은 무엇일까요? 여러분은 어떤 까닭으로 그렇게 생각하시나요?"',
              ],
            ],
            '하나의 기기를 AI 또는 비AI로 단순하게 나누지 않아도 돼요.',
          ),
        },
      ],
      knowledge: [
        {
          title: '기기보다 기능을 봐요',
          core: '기기 전체가 아니라 어떤 기능을 가지고 있는지 먼저 확인합니다.',
          detail: {
            full: '한 기기 안에도 여러 가지 작동 기능이 함께 들어 있어요.',
            light: '같은 기기 안에도 버튼 기능, 센서 기능, AI 기능이 함께 있을 수 있습니다.',
            challenge: '기기의 이름이나 겉모습이 아니라 분석하려는 기능의 입력, 처리 방식, 결과를 구분합니다.',
          },
        },
        {
          title: '입력 신호가 달라요',
          core: '버튼, 센서, 타자, 음성 등으로 기계에게 요구할 수 있어요.',
          detail: {
            full: '기계가 동작하기 위해 어떤 정보를 입력받는지 찾아봐요.',
            light: '버튼은 직접 명령을, 센서는 주변 변화를, 타자와 음성은 글자나 말소리로 요구 사항을 전달합니다.',
            challenge: '입력의 종류와 방식을 살펴보면 단순 작동과 감지, 텍스트/음성 기반 처리의 차이를 설명할 수 있습니다.',
          },
          flow: { input: '버튼·센서·타자·음성', process: '작동·감지·입력 처리', output: '바람·문 열림·결과 제공' },
        },
        {
          title: '기능은 함께 쓰일 수 있어요',
          core: '자동화, 센서, AI는 한 도구에서 함께 작동할 수 있습니다.',
          detail: {
            full: '하나의 기기에 한 가지 기능만 들어 있는 것은 아니에요.',
            light: '로봇청소기처럼 센서로 보고 AI로 경로를 고르는 도구도 있습니다.',
            challenge: '기능을 겹치지 않는 상자로 나누기보다 각 기능이 맡은 역할과 사람의 확인 지점을 설명합니다.',
          },
        },
      ],
    },
    encounter: {
      title: '세 도구의 다른 작동 방식',
      description: '선풍기, 자동문, 음악 추천 앱은 모두 저마다 다른 방식으로 움직입니다. 이 세 가지 도구 중 어떤 기능에 인공지능(AI)이 쓰였는지 함께 알아봅시다.',
      facts: [
        '선풍기는 사람이 누른 버튼 신호를 받습니다.',
        '자동문은 앞에 선 사람을 센서로 감지합니다.',
        '음악 추천 앱은 이전에 들은 음악 기록을 사용합니다.',
      ],
      stimuli: [
        {
          id: 'pecs-fan',
          kind: 'image',
          src: '/images/pecs_electric_fan.webp',
          alt: '선풍기 PECS 카드 (버튼 입력)',
          caption: '선풍기 (버튼)',
        },
        {
          id: 'pecs-door',
          kind: 'image',
          src: '/images/pecs_automatic_door.webp',
          alt: '자동문 PECS 카드 (센서 감지)',
          caption: '자동문 (센서)',
        },
        {
          id: 'pecs-music-app',
          kind: 'image',
          src: '/images/pecs_music_app.webp',
          alt: '음악 추천 앱 PECS 카드 (사용 기록 입력)',
          caption: '음악 추천 앱 (사용 기록)',
        },
      ],
    },
    firstAttempt: {
      prompt: '세 가지 도구 중 어떤 것에 인공지능(AI)이 쓰였는지 어떻게 구별할 수 있을까요?',
      choices: [
        { id: 'inspect-input-output', emoji: '🔎', label: '각 기능이 무엇을 받고 어떤 결과를 내는지 살펴봅니다.', isCorrect: true, reaction: '윤아: "그거야. 받는 정보부터 보면 헷갈리지 않아."' },
        { id: 'separate-input-modes', emoji: '⚡', label: '버튼, 센서, 글자나 음성처럼 어떤 정보를 입력받느냐에 따라 처리하는 방식이 다르다는 점을 살펴봅니다.', isCorrect: true, reaction: '아이미: "제 입력은 글과 말이에요!"' },
        { id: 'automatic-means-ai', emoji: '⚙️', label: '자동으로 움직이면 모두 AI라고 생각합니다.', isCorrect: false, reaction: '윤아가 자동문을 가리켰습니다. "이건 그냥 센서로 열리는 건데?"' },
        { id: 'app-only-by-name', emoji: '📱', label: '기기 모양이나 앱 이름만 보고 정합니다.', isCorrect: false, reaction: '진우: "이름만 보고 샀다가 후회한 적 있잖아…"' },
      ],
      modes: [...STUDIO_EXPRESSION_MODES],
      reasonPrompt: '어떤 기기의 입력이 가장 궁금한가요?',
    },
    supportProfiles: STUDIO_SUPPORT_PROFILES,
    conditionChange: {
      description: '새로 본 로봇청소기는 센서로 장애물을 감지하고, 이전 청소 기록을 사용해 다음 경로를 바꿉니다.',
      facts: [
        '장애물 센서는 주변 상태를 감지합니다.',
        '청소 기록은 먼지가 많았던 위치를 남깁니다.',
        '경로 추천 기능은 센서 정보와 이전 기록을 함께 사용합니다.',
        '사람은 위험한 물건을 치우고, 청소가 잘 되었는지 최종 확인합니다.',
      ],
      stimuli: [
        {
          id: 'pecs-robot-vacuum',
          kind: 'image',
          src: '/images/pecs_robot_vacuum.webp',
          alt: '로봇청소기 PECS 카드 (센서+AI 복합)',
          caption: '로봇청소기 (센서+AI)',
        },
      ],
    },
    aiContribution: {
      source: 'prepared',
      role: '아이미의 생각',
      text: '로봇청소기와 자동문은 기계입니다. 둘 다 스스로 움직이지요. 하지만 로봇청소기만 인공지능이라고 부릅니다. 자동문과 로봇청소기가 다른 점은 무엇일까요?',
      question: '자동문과 로봇청소기가 다른 점은 무엇일까요?',
    },
    artifact: {
      kind: 'visual-plan',
      title: '기능 분석 지도',
      prompt: '도구 하나를 골라 입력, 기능, 결과, 사람이 확인할 일을 네 칸에 연결해 봐요.',
    },
    transfer: {
      title: '스마트 조명을 살펴본다면',
      description: '스마트 조명은 사람의 움직임을 감지해 불을 켜고, 평소 조명을 끄고 켜는 시간을 스스로 학습하여 알맞은 밝기를 추천해 줍니다.',
      prompt: '스마트 조명 속에 들어 있는 여러 가지 기능을 친구에게 알기 쉽게 설명해 봐요.',
      stimuli: [
        {
          id: 'smart-light-real',
          kind: 'image',
          src: '/images/smart_light_real.webp',
          alt: '스마트 조명 및 제어 단말기 사진',
          caption: '스마트 조명 (센서 + AI 기능)',
        },
      ],
      choices: [
        { id: 'separate-sensor-action', emoji: '💡', label: '움직임 감지 센서 기능과 조명이 켜지는 기본 작동을 구분하여 설명합니다.', isCorrect: true, reaction: '아이미: "정확해요! 센서와 기본 작동은 다른 기능이에요."' },
        { id: 'combine-sensor-ai', emoji: '🧩', label: '센서 감지 기능과 사용 습관을 배우는 밝기 추천 AI가 함께 작동함을 설명합니다.', isCorrect: true, reaction: '윤아: "맞아, 한 기기에 여러 기능이 같이 있을 수 있어."' },
        { id: 'call-whole-device-ai', emoji: '⚙️', label: '조명의 겉모습만 보고 기기 전체를 무조건 하나의 AI라고만 부릅니다.', isCorrect: false, reaction: '진우: "겉모습만 보면 또 헷갈리는데?"' },
        { id: 'all-electric-ai', emoji: '⚡', label: '전기로 켜지는 모든 조명은 스스로 생각하므로 무조건 AI라고 부릅니다.', isCorrect: false, reaction: '아이미: "전기로 켜진다고 다 저 같은 건 아니에요."' },
      ],
    },
  };
