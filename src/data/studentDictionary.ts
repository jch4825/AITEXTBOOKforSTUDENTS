...
export const STUDENT_DICTIONARY: DictionaryEntry[] = [
  ...
  },
  {
    ...
    ttsVersion: '수행은 맡은 일이나 명령을 실제로 처리해내는 것입니다.',
  },
  {
    term: '범주',
    aliases: ['종류', '분류'],
  ...
  {
    // m2-l8이 "시간은 표, 순서는 번호 목록"으로 형식을 가르치면서 사전 용어로 선언한다.
    // 한 글자라 낱말 안에서 오검출되므로 lessonTextUtils의 자동 밑줄 제외 목록에 이미 들어 있고,
    // 이 항목은 쉬운 사전에서 직접 찾아볼 때 쓰인다.
    term: '표',
    shortExplanation: '칸을 나누어 여러 가지를 한눈에 보이게 정리한 것입니다.',
    example: '요일과 시간을 칸에 넣어 시간표를 만듭니다.',
    ttsVersion: '표는 칸을 나누어 여러 가지를 한눈에 보이게 정리한 것입니다.',
  },
  {
    term: '요소',
