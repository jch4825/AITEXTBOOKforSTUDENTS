...
interface FlagshipLessonViewProps {
  lesson: CanonicalLessonDesign;
  supportLevel: SupportLevel;
  onFinish?: () => void;
}

export const FlagshipLessonView: React.FC<FlagshipLessonViewProps> = ({
  lesson,
  supportLevel,
  onFinish,
}) => {
  ...
  return (
    <div className="w-full max-w-[min(96vw,110rem)] 2xl:max-w-[min(94vw,148rem)] 3xl:max-w-[min(92vw,175rem)] mx-auto p-4 sm:p-6 space-y-6">
      ...
      <div className="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          <div className="flex items-center space-x-2">
            <span className="px-2.5 py-0.5 rounded-full bg-indigo-100 text-indigo-800 text-xs font-extrabold uppercase">
              경험 중심 플래그십
            </span>
      ...
      {/* Main Digital Comic Spread Layout (Desktop: 2 Columns, Mobile: 1 Column) */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        {/* Left Page: Story, Scene & Evidence Assets */}
        <div className="lg:col-span-5 space-y-4">
          ...
          <LessonEvidencePanel
            ...
            changedReason={session.changedReason}
          />
        </div>

        {/* Right Page: Activity & Interaction */}
