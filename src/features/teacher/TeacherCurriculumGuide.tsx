...
export const DETAILED_STANDARD_EXPLANATIONS: Record<string, string> = {
  ...
  // === 영역 4. 인공지능 안전과 윤리 ===
  '[9인지04-01]':
    '이 성취기준은 이름이나 연락처 같은 직접 단서뿐 아니라 학교 이름, 반복하는 하교 시간, 사진 배경의 이름표와 위치 표지처럼 여러 개가 모이면 개인을 알아볼 수 있는 간접 단서까지 찾아 가리고, 비밀번호와 인증 코드를 요구하는 메시지에는 답하지 않고 멈추어 알리는 실천 능력을 기르기 위해 설정하였다. 4단원 3차시 「개인정보는 가리고 필요한 조건만 말하기」에서는 포스터 도움을 구하는 채팅 초안에서 가릴 정보 묶음을 고르고 작업 조건만 남긴 요청으로 고쳐 「가리기 전후 안전 요청」을 작성하며, 화면 활동 「개인정보 스티커 붙이기」에서는 사진 속 내 얼굴, 이름표, 학교 로고, 집 번호를 눌러 가린 뒤 레이더 검사로 남은 단서를 확인한다. 4단원 4차시 「비밀번호와 개인 인증 정보는 보내지 않기」에서는 인증 코드 요구에 거절과 화면 닫기와 알리기를 순서대로 수행하고 학교 공식 안내에서 복구 경로를 확인하며, 5차시의 사진 단서 점검과 9차시의 위험 요청 알리기로 같은 절차를 다른 상황에 넓혀 적용한다.',
  '[9인지04-02]':
    '이 성취기준은 불편한 화면을 만났을 때 멈추어 알리고 존중하는 표현으로 요청하며, 정해진 숫자를 지키기보다 자신의 몸 신호와 일정에 맞는 사용 계획을 세워 조절하려는 마음을 기르기 위해 설정하였다. 4단원 8차시 「인공지능을 멈추고 쉴 시간 정하기」에서는 매일 30분처럼 모두에게 같은 숫자를 정하는 방법이 주말 가족 일정에서 곧바로 어긋나는 과정을 확인한 뒤, 알람이 울려도 다음 행동을 몰라 준비 시간을 놓쳤던 기록을 근거로 멈춤 신호와 종료 뒤 행동, 평일·주말 조정, 도움받을 사람을 담은 「개인 사용·휴식 계획」을 작성하며, 화면 활동 「멈춤 시계 쉼표」에서 화면 블록이 길게 이어지기 전에 쉼표를 끼운다. 4단원 6차시에서는 안전 덮개로 가려진 화면을 다시 읽지 않고 멈춤과 가리기와 거리두기와 알리기를 순서대로 연습하고, 7차시에서는 목적·행동·조건·존중 표현을 넣어 요청을 고치며, 10차시에서는 추천 게시물의 협찬 표시와 구매 링크와 과장을 찾아 자신의 필요·대안·예산과 비교하여 수용·보류·거절을 판단한다.',
  '[12인지04-01]':
    '이 성취기준은 글과 사진에 담긴 개인 식별 단서를 찾아 가리고, 자료를 쓴 사람과 게시 날짜를 지금 상황과 대조하여 근거로 쓸 자료를 가려내는 보안 역량을 기르기 위해 설정하였다. 4단원 2차시 「출처와 날짜를 보고 믿을 만한 자료 고르기」에서는 쓴 사람을 알 수 없는 캡처와 지난달 다른 행사의 우천 취소 공지와 오늘 담당 부서의 정상 운영 공지를 쓴 사람·날짜·원문 확인 가능성이라는 같은 기준으로 비교하여 출처 비교 카드를 작성하고, 1차시에서는 지난주 초안을 근거로 답한 인공지능의 시각과 오늘 게시된 최종 시간표를 대조하여 맞는 부분과 고칠 부분을 나눈다. 4차시에서는 인증 코드를 요구하는 메시지를 학교 홈페이지의 공식 도움 안내와 대조하여 진짜 도움 경로를 가려낸다. 3차시에서는 채팅 초안의 직접 단서와 간접 단서를 구분해 가리고 작업 조건만 남기며, 5차시 「사진을 올리기 전에 한 번 더 살펴보기」에서는 이름표·교실 위치 표지·친구 얼굴·내일 일정표를 확인하고 유리창에 비친 얼굴과 교실 번호까지 살펴 그대로 보내기와 가리고 보내기와 보내지 않기 가운데 하나를 이유와 함께 결정한다.',
  '[12인지04-02]':
    '이 성취기준은 나의 정보와 함께 다른 사람의 정보와 동의를 존중하고, 디지털 공간에서 분명하고 존중하는 표현을 사용하며, 미디어 이용을 스스로 조절하려는 태도를 기르기 위해 설정하였다. 4단원 9차시 「위험하거나 이상한 요구는 어른에게 알리기」에서는 선물과 비밀과 만남을 요구하는 낯선 계정에 멈춤과 거절과 차단을 실행한 뒤 지금 알릴 사람과 그 사람이 안 될 때 알릴 다음 사람까지 적어 개인 도움망을 완성하고, 알리는 일이 고자질이 아니라 자기보호임을 확인한다. 8차시에서는 눈 피로와 놓친 포스터 정리 시간을 자기 멈춤 신호로 삼아 평일과 주말을 다르게 조정한 「개인 사용·휴식 계획」을 세우고, 7차시에서는 무례한 요청을 목적·행동·조건·존중 표현이 담긴 부탁으로 고쳐 친구에게 색종이를 빌리는 상황까지 넓히며, 6차시에서는 불편한 화면을 다시 열지 않고도 도움을 요청할 수 있음을 확인한다. 다른 사람의 얼굴과 동의를 다루는 5차시 활동은 이 성취기준이 아니라 [12인지04-01]로 태깅되어 있으므로 해설의 근거 차시로 인용하지 않는다.',
    ...
    <div className="space-y-6 text-slate-800">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-xs sm:text-sm text-slate-300 max-w-3xl leading-relaxed">
              국가가 정한 선택 교과 세 과목에는 인공지능을 다룰 자리가 없습니다. 거제애광학교가 학교 자율 교과로 따로 만든 「인공지능 활용」의 교육과정 명세입니다.
            </p>
          </div>
      ...
      <nav aria-label="교육과정 문서 차례" className="bg-white rounded-2xl p-5 md:p-6 border border-slate-200 depth-paper">
        ...
        <ol className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3 text-xs font-bold text-slate-800">
          {[
            { label: '1. 교육과정 설계의 개요', note: '학교 자율 교과로 만든 까닭' },
      ...
      <section className="bg-white rounded-2xl p-6 md:p-8 border border-slate-200 depth-paper space-y-4">
        <div className="flex items-center gap-2 border-b border-slate-200 pb-3">
          ...
          <h2 className="text-xl font-extrabold text-slate-900">1. 교육과정 설계의 개요</h2>
        </div>
        <div className="text-sm text-slate-700 leading-relaxed space-y-3 font-medium">
          <div className="p-4 rounded-xl bg-amber-50 border border-amber-200 text-slate-900 leading-relaxed space-y-2">
            <p className="font-extrabold text-amber-950 flex items-center gap-1.5 text-sm sm:text-base">
              <span>🏫</span> <span>거제애광학교가 이 과목을 만든 까닭</span>
            </p>
            <p className="text-xs sm:text-sm text-slate-800 leading-relaxed">
              기본 교육과정이 정한 선택 교과는 정보통신활용, 생활영어, 보건 세 과목입니다. 학생이 인공지능을 직접 다루어 볼 시간은 여기에 없습니다. 그래서 <strong>거제애광학교</strong>가 <strong>「인공지능 활용」</strong>을 학교 자율 교과로 만들었습니다.
            </p>
          </div>
          <p>
            학생이 인공지능 기기와 소프트웨어를 직접 써 보고, 돌아온 답을 확인해 쓸지 고칠지 말지 스스로 정해 보는 생활 중심 과목입니다.
          </p>
          <p>
            문서의 짜임은 기본 교육과정의 교과 문서를 따랐습니다. 내용 체계는 <strong>지식·이해</strong>와 <strong>과정·기능</strong>, <strong>가치·태도</strong>의 세 범주로 나누고, 문서는 성격과 목표, 내용 체계, 성취기준, 교수·학습과 평가의 차례로 이어집니다.
          </p>
          <div className="p-4 rounded-xl bg-rose-50 border border-rose-200 text-slate-900 leading-relaxed space-y-1.5">
            ...
            <p className="text-xs sm:text-sm text-slate-800 leading-relaxed">
              이 과목의 <strong>[9인지00-00]·[12인지00-00]</strong> 코드는 거제애광학교가 스스로 붙인 학교 자율 교과의 코드이며 <strong>국가 수준 성취기준이 아닙니다</strong>. 생활기록부·개별화교육계획에 기재할 때에는 학교 자율 교과의 성취기준임을 함께 밝히고, 국가 교육과정 코드와 혼동되지 않도록 표기합니다. 국가 기본 교육과정 교과와의 연계는 교사 모드의 <strong>연계 성취기준</strong> 탭에서 확인합니다.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="bg-white rounded-2xl p-6 md:p-8 border border-slate-200 depth-paper space-y-4">
          ...
          <table className="w-full min-w-[640px] text-xs text-left border-collapse border border-slate-300">
            ...
            <tbody className="divide-y divide-slate-300 font-medium">
              <tr className="bg-amber-50/40">
                ...
                <td className="p-3 text-slate-800">정보와 선택지를 줄이고 시각 단서를 더해 곁에서 직접 돕습니다.</td>
              </tr>
              ...
              <tr className="bg-emerald-50/40">
                ...
                <td className="p-3 text-slate-800">전이 단계에 <strong>고등 심화 과제</strong>가 하나 더 붙습니다. 같은 이야기를 상황 복잡도·자료 수·독립성·전이 범위 가운데 하나 이상을 올려 다시 해냅니다.</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 text-xs sm:text-sm text-slate-800 space-y-1.5">
          ...
          <ul className="list-inside list-disc space-y-1 font-medium leading-relaxed">
            ...
            <li>교실 밖에서 해야 하는 고등 심화 과제의 인솔·안전 계획 — 단원마다 한두 차시에 지역사회 수행이 들어 있습니다.</li>
          </ul>
          <p className="pt-1 text-slate-600">이 세 가지는 학교가 정할 몫이므로 이 명세에서는 값을 못 박지 않았습니다.</p>
        </div>
      </section>
        ...
        <div className="space-y-4 text-sm">
          <div>
            ...
            <p className="text-slate-700 leading-relaxed font-medium">
              <strong>「인공지능 활용」</strong>은 학생이 날마다 마주치는 인공지능이 어떻게 답을 만드는지 알고, 기기와 소프트웨어를 다루어 생활 속 문제를 푸는 힘을 기르는 선택 교과이다. 학생은 인공지능과 주고받아 본 경험에서 그 답이 사실인지 가려내고, 자신과 남의 정보를 지키며 바르게 쓰는 태도를 익힌다.
            </p>
          </div>
          ...
          <div>
            ...
            <p className="text-slate-900 font-extrabold mb-3 leading-relaxed bg-amber-50/80 p-3.5 rounded-xl border border-amber-200/90 text-sm sm:text-base">
              인공지능의 기초 지식과 기본 기능을 익혀 바르게 쓰고, 절차를 세워 생활 속 문제를 풀며, 자신과 남의 정보를 지켜 안전하게 쓰는 힘을 기른다.
            </p>
      ...
      <section id="curriculum-contents" className="bg-white rounded-2xl p-6 md:p-8 border border-slate-200 depth-paper space-y-6">
        ...
        <div>
          ...
          <p className="mt-1 text-xs sm:text-sm text-slate-600 font-medium leading-relaxed">
            여섯 영역마다 핵심 아이디어를 앞에 두고, 지식·이해와 과정·기능, 가치·태도의 세 범주로 학년군별 내용 요소를 적었습니다.
          </p>
        </div>
        ...
        <div className="space-y-8 divide-y divide-slate-200 pt-2">
          ...
          <div className="space-y-3 pt-4 first:pt-0">
            ...
            <div className="p-3.5 rounded-xl bg-indigo-950 text-white space-y-1.5 border border-indigo-800">
              ...
              <ul className="list-inside list-disc text-xs text-slate-200 space-y-1 leading-relaxed pl-1">
                <li>인공지능은 사람이 모아 준 자료에서 규칙을 찾아 답을 만들며, 그 자료가 닿지 않는 곳은 알지 못한다.</li>
                <li>인공지능의 답에는 그럴듯한 거짓(환각)이 섞일 수 있으므로, 원본 자료와 견주어 확인한 뒤에 쓴다.</li>
              </ul>
            </div>

            {/* 내용 체계 표 */}
            <div className="overflow-x-auto">
              <table className="w-full min-w-[600px] text-xs text-left border-collapse border border-slate-300">
                <thead>
                  <tr className="bg-slate-100 text-slate-900 font-extrabold border-b border-slate-300">
                ...
                <tbody className="divide-y divide-slate-300 font-medium">
                  <tr className="bg-amber-50/30">
                    ...
                    <td className="p-2.5 border-r border-slate-300 text-slate-800">• 인공지능이 답을 만드는 방식<br />• 사람이 모아 준 자료와 답의 관계</td>
                    <td className="p-2.5 text-slate-800">• 인공지능의 개념과 원리<br />• 자료를 익혀 답을 만드는 원리<br />• 그럴듯한 거짓(환각)의 특성</td>
                  </tr>
          ...
          <div className="space-y-3 pt-6">
            ...
            <div className="p-3.5 rounded-xl bg-indigo-950 text-white space-y-1.5 border border-indigo-800">
              ...
              <ul className="list-inside list-disc text-xs text-slate-200 space-y-1 leading-relaxed pl-1">
                <li>인공지능에게 건네는 이름과 조건, 예시가 구체적일수록 돌아오는 답이 또렷해진다.</li>
                <li>큰 요청은 한 번에 하지 않고 단계로 나누어 건네며, 첫 답을 확인한 뒤 조건을 보태어 다시 요청한다.</li>
              </ul>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[600px] text-xs text-left border-collapse border border-slate-300">
                ...
                <tbody className="divide-y divide-slate-300 font-medium">
                  <tr className="bg-amber-50/30">
                    <td className="p-3 border-r border-slate-300 font-black text-amber-900 text-center bg-amber-100/60">지식 · 이해</td>
                    <td className="p-2.5 border-r border-slate-300 text-slate-800">• 인공지능에게 주는 낱말과 조건의 뜻<br />• 조건이 바뀌면 답이 달라지는 까닭</td>
                    <td className="p-2.5 text-slate-800">• 이름·조건·예시를 구체적으로 담는 방법<br />• 요청이 구체적일수록 또렷해지는 답</td>
                  </tr>
                  <tr className="bg-sky-50/30">
                    <td className="p-3 border-r border-slate-300 font-black text-sky-900 text-center bg-sky-100/60">과정 · 기능</td>
                    <td className="p-2.5 border-r border-slate-300 text-slate-800">• 구체적인 이름과 조건을 더해 다시 요청하기<br />• 원하는 대답을 얻기 위해 입력 낱말 수정하기</td>
                    <td className="p-2.5 text-slate-800">• 과제를 단계로 나누어 차례로 요청하기<br />• 원하는 답의 예시를 넣어 요청 고치기</td>
                  </tr>
                  <tr className="bg-emerald-50/30">
                    <td className="p-3 border-r border-slate-300 font-black text-emerald-900 text-center bg-emerald-100/60">가치 · 태도</td>
                    <td className="p-2.5 border-r border-slate-300 text-slate-800">• 내 생각을 분명하게 전달하려는 태도<br />• 인공지능에게 예의 바르게 요청하는 자세</td>
                    <td className="p-2.5 text-slate-800">• 뜻을 분명하게 밝히려는 태도<br />• 답이 어긋나도 다시 요청해 보는 자세</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div className="space-y-3 pt-6">
            ...
            <div className="p-3.5 rounded-xl bg-indigo-950 text-white space-y-1.5 border border-indigo-800">
              ...
              <ul className="list-inside list-disc text-xs text-slate-200 space-y-1 leading-relaxed pl-1">
                <li>인공지능은 모르는 것을 알아보도록 돕는 학습 도구이며, 무엇을 배울지 정하고 판단하는 몫은 학생에게 있다.</li>
                <li>인공지능의 설명은 참고로 삼고, 자기 말로 다시 정리해야 자기 것이 된다.</li>
              </ul>
            </div>
              ...
              <table className="w-full min-w-[600px] text-xs text-left border-collapse border border-slate-300">
                ...
                <tbody className="divide-y divide-slate-300 font-medium">
                  <tr className="bg-amber-50/30">
                    ...
                    <td className="p-2.5 text-slate-800">• 공부에서 인공지능이 도울 수 있는 일<br />• 학생이 직접 맡아야 할 몫</td>
                  </tr>
                  <tr className="bg-sky-50/30">
                    <td className="p-3 border-r border-slate-300 font-black text-sky-900 text-center bg-sky-100/60">과정 · 기능</td>
                    <td className="p-2.5 border-r border-slate-300 text-slate-800">• 모르는 낱말이나 풀이 인공지능에게 물어보기<br />• 나온 설명을 확인하여 이해하고 정리하기</td>
                    <td className="p-2.5 text-slate-800">• 배운 내용과 낱말 뜻을 물어 설명 얻기<br />• 얻은 설명을 자기 말로 다시 적기</td>
                  </tr>
                  <tr className="bg-emerald-50/30">
                    <td className="p-3 border-r border-slate-300 font-black text-emerald-900 text-center bg-emerald-100/60">가치 · 태도</td>
                    <td className="p-2.5 border-r border-slate-300 text-slate-800">• 도움을 받으면서도 스스로 풀어 보려는 마음<br />• 끝까지 해 보려는 의지</td>
                    <td className="p-2.5 text-slate-800">• 인공지능에 기대어 판단을 넘기지 않는 태도<br />• 스스로 먼저 생각하고 확인하는 자세</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div className="space-y-3 pt-6">
            ...
            <div className="p-3.5 rounded-xl bg-indigo-950 text-white space-y-1.5 border border-indigo-800">
              ...
              <ul className="list-inside list-disc text-xs text-slate-200 space-y-1 leading-relaxed pl-1">
                <li>이름과 얼굴처럼 나를 드러내는 단서는 여럿이 모이면 개인을 알아볼 수 있으므로, 인공지능에 넘기기 전에 가린다.</li>
                <li>위험한 요청 앞에서는 멈추어 믿을 만한 어른에게 알리고, 사용 시간은 자신의 몸 상태와 일정에 맞추어 조절한다.</li>
              </ul>
            </div>
              ...
              <table className="w-full min-w-[600px] text-xs text-left border-collapse border border-slate-300">
                ...
                <tbody className="divide-y divide-slate-300 font-medium">
                  <tr className="bg-amber-50/30">
                    ...
                    <td className="p-2.5 text-slate-800">• 여럿이 모이면 개인을 드러내는 단서<br />• 낚시 메시지와 숨은 광고의 특징</td>
                  </tr>
                  <tr className="bg-sky-50/30">
                    <td className="p-3 border-r border-slate-300 font-black text-sky-900 text-center bg-sky-100/60">과정 · 기능</td>
                    <td className="p-2.5 border-r border-slate-300 text-slate-800">• 사진이나 글에서 개인정보 찾아 가리기<br />• 위험한 요청을 만나면 멈추고 알리기</td>
                    <td className="p-2.5 text-slate-800">• 개인을 드러내는 단서 찾아 가리기<br />• 자료의 출처와 날짜를 대조해 쓰기</td>
                  </tr>
                  <tr className="bg-emerald-50/30">
                    <td className="p-3 border-r border-slate-300 font-black text-emerald-900 text-center bg-emerald-100/60">가치 · 태도</td>
                    <td className="p-2.5 border-r border-slate-300 text-slate-800">• 내 정보를 스스로 지키려는 마음<br />• 정한 사용 시간을 지키려는 자세</td>
                    <td className="p-2.5 text-slate-800">• 남의 정보도 내 정보처럼 지키는 태도<br />• 사용 시간을 스스로 조절하는 태도</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div className="space-y-3 pt-6">
            ...
            <div className="p-3.5 rounded-xl bg-indigo-950 text-white space-y-1.5 border border-indigo-800">
              ...
              <ul className="list-inside list-disc text-xs text-slate-200 space-y-1 leading-relaxed pl-1">
                <li>문제는 지금 상태와 이루고 싶은 상태의 차이에서 드러나며, 그 차이를 한 문장으로 적을 때 풀 수 있는 문제가 된다.</li>
                <li>큰 과제는 작게 나누어 먼저 할 일을 정하고, 틀린 곳은 힌트를 받아 끝까지 고친다.</li>
              </ul>
            </div>
              ...
              <table className="w-full min-w-[600px] text-xs text-left border-collapse border border-slate-300">
                ...
                <tbody className="divide-y divide-slate-300 font-medium">
                  <tr className="bg-amber-50/30">
                    ...
                    <td className="p-2.5 text-slate-800">• 지금 상태와 목표를 견주어 문제 정하기<br />• 순서를 세워 푸는 절차의 개념</td>
                  </tr>
                  <tr className="bg-sky-50/30">
                    <td className="p-3 border-r border-slate-300 font-black text-sky-900 text-center bg-sky-100/60">과정 · 기능</td>
                    <td className="p-2.5 border-r border-slate-300 text-slate-800">• 목표를 위해 필요한 순서 차례대로 정하기<br />• 힌트를 받아 틀린 부분 찾아 고쳐 보기</td>
                    <td className="p-2.5 text-slate-800">• 큰 문제를 작은 과제로 나누어 순서 정하기<br />• 힌트를 받아 틀린 곳 고치기</td>
                  </tr>
                  <tr className="bg-emerald-50/30">
                    <td className="p-3 border-r border-slate-300 font-black text-emerald-900 text-center bg-emerald-100/60">가치 · 태도</td>
                    <td className="p-2.5 border-r border-slate-300 text-slate-800">• 틀려도 포기하지 않고 다시 해보는 자세<br />• 순서대로 차근차근 문제를 해결하려는 마음</td>
                    <td className="p-2.5 text-slate-800">• 틀린 곳을 끈기 있게 고치려는 태도<br />• 막혀도 다른 방법을 찾아보려는 자세</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div className="space-y-3 pt-6">
            ...
            <div className="p-3.5 rounded-xl bg-indigo-950 text-white space-y-1.5 border border-indigo-800">
              ...
              <ul className="list-inside list-disc text-xs text-slate-200 space-y-1 leading-relaxed pl-1">
                <li>장보기와 이동, 날씨처럼 날마다 달라지는 정보는 인공지능의 한마디로 정하지 않고 공식 자료와 견주어 확인한다.</li>
                <li>일정과 이동, 식단과 건강을 인공지능의 도움으로 스스로 챙길 때 지역사회에서 자기 삶을 꾸릴 수 있다.</li>
              </ul>
            </div>
              ...
              <table className="w-full min-w-[600px] text-xs text-left border-collapse border border-slate-300">
                ...
                <tbody className="divide-y divide-slate-300 font-medium">
                  <tr className="bg-amber-50/30">
                    ...
                    <td className="p-2.5 text-slate-800">• 지역사회에서 만나는 무인 단말기와 교통 안내<br />• 생활 속 인공지능 도구의 종류와 특징</td>
                  </tr>
                  <tr className="bg-sky-50/30">
                    <td className="p-3 border-r border-slate-300 font-black text-sky-900 text-center bg-sky-100/60">과정 · 기능</td>
                    <td className="p-2.5 border-r border-slate-300 text-slate-800">• 날씨와 일정을 찾아 공식 자료로 확인하기<br />• 도움을 받아 화면에서 필요한 정보 고르기</td>
                    <td className="p-2.5 text-slate-800">• 일정과 이동, 식단을 도구로 챙기기<br />• 계획이 막히면 다른 방법으로 바꾸기</td>
                  </tr>
                  <tr className="bg-emerald-50/30">
                    <td className="p-3 border-r border-slate-300 font-black text-emerald-900 text-center bg-emerald-100/60">가치 · 태도</td>
                    <td className="p-2.5 border-r border-slate-300 text-slate-800">• 일상에서 인공지능을 스스로 활용하여 생활하기<br />• 인공지능 도구를 기분 좋게 활용하려는 자세</td>
                    <td className="p-2.5 text-slate-800">• 마지막 결정을 스스로 내리려는 자세<br />• 지역사회에서 자기 삶을 꾸리려는 태도</td>
                  </tr>
                </tbody>
              </table>
            </div>
        ...
        <div className="space-y-4 border-t border-slate-200 pt-6">
          <div>
            ...
            <p className="mt-1 text-xs sm:text-sm text-slate-600 font-medium leading-relaxed">
              내용 체계의 요소를 어느 차시 묶음에서 어떻게 다루는지 단원별로 적었습니다. 차시 번호는 학생 화면의 차시와 같습니다.
            </p>
          </div>
              ...
              >
                ...
                <div className="bg-sky-50/70 p-4 rounded-xl border border-sky-200/80 text-xs sm:text-sm text-slate-800 space-y-1.5">
                  ...
                  <ul className="list-inside list-disc space-y-1 text-slate-800 font-medium pl-2 leading-relaxed text-xs">
                    <li>
                      말로 표현하기 어려운 학생에게는 보완대체의사소통(AAC) 카드와 낱말 고르기 상자, 음성 입력과 출력을 함께 쓴다.
                    </li>
                    <li>
                      복잡한 과제는 한 번에 지시하지 않고 한 번에 한 단계씩 내주며, 단계마다 시각 힌트를 함께 붙인다.
                    </li>
                    <li>
                      기본 수업은 미리 검수한 예시만으로 진행하여 카메라와 마이크 권한 없이 끝맺는다.
                    </li>
                    <li>
                      실시간 AI를 연결하면 학생이 보낸 질문과 음성, 사진이 바깥 사업자의 서버로 나간다. 보호자 동의를 받고, 얼굴과 이름과 연락처를 지운 것을 확인한 뒤에만 쓴다.
                    </li>
                  </ul>
                </div>
              </div>
