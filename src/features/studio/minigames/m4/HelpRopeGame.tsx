    ...
    >
      <div className="flex min-h-0 flex-1 flex-col gap-2.5">
        <section className="rounded-xl border-4 border-red-400 bg-red-950 px-3 py-2 text-center" aria-label="위험한 메시지 장면">
          ...
          <p className="mt-1 text-[14px] font-bold text-red-100">화면을 닫고 믿을 만한 어른에게 연결해요.</p>
        </section>
        ...
        <section className="rounded-xl border-2 border-emerald-300/60 bg-emerald-950/45 p-2.5" aria-label="도움 밧줄 장면">
          <div className="flex min-h-20 items-center gap-1.5 overflow-x-auto rounded-lg border-2 border-dashed border-emerald-200/60 bg-slate-950/45 p-2">
            <span className="shrink-0 rounded-lg border-2 border-red-300 bg-red-950 px-2 py-2 text-[14px] font-black text-white">📱 나</span>
            {passed.map((id) => { const item = STEPS.find((step) => step.id === id)!; return <React.Fragment key={id}><span className="text-lg text-emerald-300" aria-hidden="true">→</span><div className="min-w-24 shrink-0 rounded-lg border-2 border-emerald-300 bg-emerald-900/70 px-2 py-2 text-center text-[14px] font-black text-white"><span className="block text-lg" aria-hidden="true">{item.icon}</span>{item.label}</div></React.Fragment>; })}
            <span className="text-lg text-emerald-300" aria-hidden="true">→</span><span className="shrink-0 rounded-lg border-2 border-violet-300 bg-violet-950 px-2 py-2 text-[14px] font-black text-white">👩‍🏫 어른</span>
          </div>
          <p className="mt-1 text-center text-[14px] font-bold text-emerald-100">{selected ? `${selected.icon} ${selected.scene}` : passed.length === STEPS.length ? stage.response : '카드를 골라 밧줄 위에 전달해 보세요.'}</p>
        </section>
      </div>
    </MiniGameFrame>
  );
