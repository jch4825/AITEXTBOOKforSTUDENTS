...
export const Lesson43Interactive = ({ onExecute }: { onExecute: (data: {title: string, content: React.ReactNode, point: string, hideDocsButton?: boolean}) => void }) => {
  const canvasPrompt = `초등학교 5학년 수학 수업에서 쓸 수 있는 분수 덧셈 자동 채점 게임을 만들어줘:
1) 분모가 같은 진분수 덧셈 문제 5개를 랜덤으로 출제
2) 학생이 답을 분수 형태로 입력하면 정답/오답을 바로 표시
3) 맞힌 개수와 점수를 자동 계산
4) 틀린 문제는 정답과 간단한 풀이를 보여주기
5) "새 문제 시작" 버튼으로 다시 풀 수 있게 만들기
색감은 밝고 집중하기 쉬운 교실 활동용 화면으로 만들어줘.`;

  const handleTest = () => {
    onExecute({
      title: 'Gemini Canvas 체험 시뮬레이션 결과',
      content: `### 🎨 Gemini Canvas가 즉시 생성하는 화면 (예상)\n\n\`\`\`text\n[Canvas 미리보기]\n\n┌─────────────────────────────────────┐\n│  🧮 분수 덧셈 챌린지                │\n├─────────────────────────────────────┤\n│  문제 1 / 5                         │\n│                                     │\n│      2/7 + 3/7 = [ 5/7      ]       │\n│                                     │\n│  [ 채점하기 ]                       │\n│                                     │\n│  ✅ 정답입니다!                     │\n│                                     │\n│  현재 점수: 1 / 5                   │\n│  진행률: ■□□□□                     │\n│                                     │\n│  [ 다음 문제 ]  [ 새 문제 시작 ]    │\n└─────────────────────────────────────┘\n\n오답일 때는 "분모는 그대로 두고 분자끼리 더해요: 2+3=5"처럼 짧은 풀이가 함께 표시됩니다.\n수정하고 싶으면 "문제를 10개로 늘려줘", "난이도를 조금 올려줘"처럼 말로 조정하면 됩니다.\n\`\`\``,
      point: '단 한 번의 자연어 지시만으로 문제 생성·입력창·자동 채점·점수 계산이 작동하는 수업용 미니 게임이 만들어집니다. 이것이 가장 낮은 진입 장벽의 바이브 코딩이며, 다음 4-4에서는 이를 영구 재사용 가능한 "앱"으로 확장합니다.',
      hideDocsButton: true
    });
  };
