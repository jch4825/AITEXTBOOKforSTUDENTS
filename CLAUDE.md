...
- 핵심 텍스트는 종이 면에서 7:1 이상의 대비를 사용하며, 모듈 강조색은 테두리·립·큰 장식에만
  ...
  컴포넌트에 직접 적지 않습니다.
- 다크 보드는 `.mini-game-board` 안의 게임 장면으로만 허용합니다. 보드 안에서도 불투명한
  `--board-*` 토큰과 2px 경계를 사용합니다.
...
- 뜻과 형태를 짝지어 둡니다. 학생이 조종하는 것은 노랑 원, 목표는 파랑 사각형,
  ...
  둘이 같은 회색입니다.
- 색상은 바우하우스에서 가져오되 명도는 대비 계약에 맞춥니다. 종이 위 노랑은 대비가
  1.70이라 도형의 최소 3:1에도 못 미치므로 **반드시 검정 윤곽선과 함께** 씁니다.
...
- `npm run check:game-visual`이 이를 강제합니다. 62개를 한 번에 바꿀 수 없으므로
  이 검사는 **전환을 마친 파일**(스크립트의 `MIGRATED`)에만 규칙을 온전히 걸고, 나머지는
  임의 색상·이모지 수가 기준선보다 늘지 않게 막는 래칫으로 동작합니다. 파일을 전환할
  때마다 `MIGRATED`에 이름을 올리고 기준선을 함께 낮춥니다.
