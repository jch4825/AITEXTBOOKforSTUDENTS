...
const { TOOLS, LESSON_TOOLS, LESSON_VIDEO, getTeacherResources } = resources;
const allLessonIds = [...roles.STUDIO_LESSON_IDS, ...roles.MODULE_CLOSE_LESSON_IDS];

// 배정된 차시가 실제로 존재해야 한다.
for (const lessonId of [...Object.keys(LESSON_TOOLS), ...Object.keys(LESSON_VIDEO)]) {
  assert(allLessonIds.includes(lessonId), `${lessonId}: 존재하지 않는 차시에 자료가 배정됐다`);
}

// 링크는 전체 URL이어야 한다. 단축 URL은 무엇을 여는지 교사가 알 수 없다.
// 호스트를 정확히 비교한다. 부분 문자열로 보면 padlet.com 안의 t.co 를 오탐한다.
const SHORTENER_HOSTS = new Set([
  'youtu.be', 'bit.ly', 'tinyurl.com', 't.co', 'han.gl', 'url.kr', 'goo.gl', 'me2.do',
]);
const isShortener = (url) => {
  try { return SHORTENER_HOSTS.has(new URL(url).hostname.replace(/^www\./, '')); }
  catch { return false; }
};
const seen = new Set();
for (const lessonId of allLessonIds) {
  for (const link of getTeacherResources(lessonId)) {
    ...
    assert(!isShortener(link.url), `${lessonId}: ${link.url} 는 단축 URL이다 — 전체 주소를 쓴다`);
...
const usedTools = new Set(Object.values(LESSON_TOOLS).flat().map((entry) => entry.tool));
for (const key of Object.keys(TOOLS)) {
  assert(usedTools.has(key), `TOOLS.${key}: 어느 차시에서도 쓰지 않는다`);
}

// 자동 요청에 403을 주는 사이트가 있다. 서버가 살아 있으면서 봇만 막는 것이므로
// 죽은 링크와 구분해 온라인 확인에서 제외한다. 사람이 열면 정상으로 동작한다.
const botProtected = new Set(
  Object.values(TOOLS).filter((tool) => tool.botProtected).map((tool) => tool.url),
);

if (online) {
  for (const url of seen) {
    if (botProtected.has(url)) continue;
    try {
...
console.log(
  `teacher resources: 도구 ${Object.keys(TOOLS).length}종, `
  + `배정 ${Object.values(LESSON_TOOLS).flat().length}건 / ${Object.keys(LESSON_TOOLS).length}차시, `
