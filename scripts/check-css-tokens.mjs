...
const RUNTIME_INJECTED = {
  ...
  // 마지막 장면 연출은 FinalSceneCelebration.tsx가 파티클마다 여섯 값을 인라인으로 준다.
  // 예전 이름은 --sparkle-*이었고, 이름이 --fs-*로 바뀐 뒤에도 이 표를 고치지 않아
  // 검사가 계속 실패하고 있었다. 실패가 상시화되면 진짜 토큰 누락이 묻힌다.
  '--fs-size': '마지막 장면 파티클이 크기를 주입',
  '--fs-duration': '마지막 장면 파티클이 재생 시간을 주입',
  '--fs-delay': '마지막 장면 파티클이 지연 시간을 주입',
  '--fs-rotation': '마지막 장면 파티클이 회전값을 주입',
  '--fs-travel': '마지막 장면 파티클이 이동 거리를 주입',
  '--fs-drift': '마지막 장면 파티클이 좌우 흔들림을 주입',
  '--dx': '폭죽 파티클이 좌표를 주입',
  '--dy': '폭죽 파티클이 좌표를 주입',
};
